...
def _find_route_regression_events(peers: Iterable[lc.Peer], leader_id: int) -> list[str]:
    events: list[str] = []
...
def _assert_no_route_regressions(peers: Iterable[lc.Peer], leader_id: int) -> None:
    events = _find_route_regression_events(peers, leader_id)
    if events:
...
def _configure_member_role(
    ...
    *,
    ...
    _wait_pattern(
        member,
        peers,
        pattern=r"(\[team-nv\] (load role=0 .*|restore member leader_suffix=)|\[team\] configured .* role=0)",
        timeout_s=boot_timeout_s,
    ...
    *,
    ...
    if skip_direct_config:
        _progress(
            ...
            f"runtimeRelayBudget={leader_status.get('runtimeRelayBudget')}"
        )
    else:
        _progress(f"configure leader direct cap: cfg direct {direct_cap}")
        _send_cfg_and_wait(
            leader,
            peers,
        ...
        leader_status = _query_cfg(leader, peers, window_s=2.0, attempts=2)
...
def _leader_observed_packet_from(text: str, *, member_id: int, leader_id: int) -> bool:
    return re.search(
        rf"\[sle-rx\]\s+(?:HELLO|HEARTBEAT|POS_REPORT|ROUTE_UPDATE|CONFIG|ACK)\s+{member_id}->{leader_id}\b",
        text,
    ) is not None
...
def _leader_observed_packet_via_relay(text: str, *, member_id: int, relay_id: int, leader_id: int) -> bool:
    return re.search(
        rf"\[sle uart client\]\s+bind member:{relay_id}\s+conn_id:\d+[\s\S]{{0,900}}?"
        rf"\[sle-rx\]\s+(?:HELLO|HEARTBEAT|POS_REPORT|ROUTE_UPDATE|CONFIG|ACK)\s+{member_id}->{leader_id}\b",
        text,
    ) is not None
    ...
    *,
    ...
    while time.time() < end:
        ...
        if pattern.search("".join(leader.log[start:])):
            return
        _send_and_collect(leader, peers, "pairing pending", min(0.5, poll_s))
...
def _wait_leader_observes_member_reboot_rejoin(
    ...
    *,
    ...
    while time.time() < end:
        ...
        record = records.get(member_id)
        if rejoin_pattern.search(text) and record is not None and record.get("online") == 1:
            return records
        time.sleep(max(0.1, poll_s - 0.6))
    raise RuntimeError(
        f"leader did not observe member={member_id} reboot HELLO/rejoin; last={last_records}"
    )
    ...
    *,
    ...
    while time.time() < end:
        ...
        for child_id in child_ids:
            pattern = rf"\[sle-rx\] (?:HELLO|HEARTBEAT|POS_REPORT) {child_id}->\d+[\s\S]*?\[state\] relay forwarded packet"
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.natural_members:
            ...
            child_relay_records = _wait_any_child_relay(
                leader,
                peers,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected-fw", default="v4.4.119")
