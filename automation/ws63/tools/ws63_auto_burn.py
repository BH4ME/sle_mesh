...
DEFAULT_EXPECTED_FW_VERSION = "v4.4.120"
        ...
        if handshake_ready:
            ...
            if not self._try_wait_for_handshake(self.wait_timeout_s):
                ...
                if not self._try_wait_for_handshake(self.manual_retry_timeout_s):
                    ...
                    logging.error(
                        "No ROM handshake after reset. If this is COM14 and the port also has no CLI reply, "
                        "there is no remaining serial-only recovery path; power-cycle or press reset, then "
                        "rerun the COM14 safe profile instead of switching to hardware RTS flashing."
                    )
                    return False
