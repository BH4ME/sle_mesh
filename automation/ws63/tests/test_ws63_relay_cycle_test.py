...
class RelayCycleUnitTest(unittest.TestCase):
    def test_parse_member_records_reads_relay_fields(self):
        text = "\n".join([
            "member=13 role=0 online=1 battery=100 fix=1 rssi=-30 relay=1 tier=1 max_down=8 last_seq=4 last_seen=10",
    ...
    def test_relay_reboot_flow_does_not_send_leave_or_child_manual_rejoin(self):
        ...
        leader_tx = [item.removeprefix("[tx] ") for item in leader.log if item.startswith("[tx] ")]
        relay_tx = [item.removeprefix("[tx] ") for item in relay.log if item.startswith("[tx] ")]
        child_tx = [item.removeprefix("[tx] ") for item in child.log if item.startswith("[tx] ")]
        ...
        self.assertGreaterEqual(offline_events[0][4], 1)
