...
class FourBoardRelayUnitTest(unittest.TestCase):
    ...
    def test_firmware_has_relay_failover_recovery_guards(self):
        ...
        self.assertIn("#define SLE_TEAM_WS2812_TEST_R 64U", app_source)
        ...
        self.assertIn("#define SLE_TEAM_WS2812_IDLE_BLINK_ON_MS 500U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_IDLE_BLINK_PERIOD_MS 1600U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_LEADER_BLINK_ON_MS 500U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_LEADER_BLINK_PERIOD_MS 1000U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_MEMBER_BLINK_ON_MS 500U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_MEMBER_BLINK_PERIOD_MS 1000U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_ERROR_BLINK_ON_MS 220U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_ERROR_BLINK_PERIOD_MS 360U", app_source)
        self.assertIn("#define SLE_TEAM_WS2812_FLASH_ON_MS 140U", app_source)
        ...
        reset_after_leave = self._extract_c_function(app_source, "static void team_member_reset_after_leave")
        self.assertIn('team_member_drop_relay_children("member-leave");', reset_after_leave)
        self.assertLess(
            reset_after_leave.index('team_member_drop_relay_children("member-leave");'),
            reset_after_leave.index("g_team_rt.role_configured = 0U;"),
        )
        self.assertIn("#define SLE_TEAM_MEMBER_UPSTREAM_RECOVER_INTERVAL_S 2U", app_source)
        self.assertIn("#define SLE_TEAM_MEMBER_UPSTREAM_STUCK_S 8U", app_source)
        self.assertIn("static void team_member_upstream_recover_after_tx_fail", app_source)
        self.assertIn('team_member_upstream_recover_after_tx_fail("not-ready"', app_source)
        self.assertIn('team_member_upstream_recover_after_tx_fail("write-fail"', app_source)
        self.assertIn("team_member_upstream_recover_tick();", app_source)
        self.assertIn("sle_uart_server_adv_restart();", app_source)
        self.assertIn("sle_uart_server_disconnect_current();", app_source)
