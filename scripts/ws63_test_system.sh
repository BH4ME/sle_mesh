...
usage() {
  cat <<USAGE
Usage:
  scripts/ws63_test_system.sh [options]
...
if [[ "$DO_UNIT" == "1" ]]; then
  ...
  run_cmd "unit_ws63_auto_burn" python3 -m unittest tools/test_ws63_auto_burn.py || FAIL_COUNT=$((FAIL_COUNT + 1))
fi
...
if [[ "$DO_LINK_CYCLE" == "1" ]]; then
  ...
  if [[ -z "$SERIAL_PORTS" ]]; then
    ...
    if [[ "${#PORT_ARR[@]}" -lt 2 ]]; then
      ...
      link_cmd=(
        python3 "$ROOT_DIR/tools/ws63_link_cycle_test.py"
  ...
  if [[ -z "$SERIAL_PORTS" ]]; then
    ...
    if [[ "${#PORT_ARR[@]}" -lt 2 ]]; then
      ...
      if [[ -z "$MEMBER_PORTS_CSV" ]]; then
        ...
        bind_cmd=(
          python3 "$ROOT_DIR/tools/ws63_flash_bind_team.py"
          --leader-port "$LEADER_PORT"
