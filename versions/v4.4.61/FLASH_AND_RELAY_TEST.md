# v4.4.61 Flash And Relay Test

## Why This File Exists

This file records the exact build, flash, and four-board relay test process so the next run shows visible progress and does not require re-analysis.
...
```text
WS63 Ubuntu build
profile:    v4.4.61 unified runtime role (conn_id zero route fix)
...
Use the multi-port script. It prints each burn line with a COM prefix and also writes a per-port log file.

Sequential flashing:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\ws63_flash_multi.ps1 -Ports COM16,COM13,COM17,COM18 -ExpectedVersion v4.4.61 -ManualRetryTimeout 90
```

Parallel flashing:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\ws63_flash_multi.ps1 -Ports COM16,COM13,COM17,COM18 -ExpectedVersion v4.4.61 -ManualRetryTimeout 90 -Parallel
```
...
3. Confirm downstream members either reconnect through a newly selected relay or return after original relay recovers.
4. Record whether original relay regains relay role or returns as a normal member.
5. Do not mark the test passed from the screen alone; use serial logs as proof.
