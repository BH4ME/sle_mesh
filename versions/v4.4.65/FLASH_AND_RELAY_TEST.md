...
```text
profile:    v4.4.65 unified runtime role
configured v4.4.65 schematic pinmap and pairing seek admission guard
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\ws63_flash_multi.ps1 `
  ...
  -ExpectedVersion v4.4.65 `
  -WaitTimeout 15 `
  -ManualRetryTimeout 300
```

Do not count a board as flashed unless `cfg status` later reports `fw:"v4.4.65"`.
...
The previous failing shape was `filter=0` for the candidate while pairing was open. That must not recur for the configured test member.
