# v4.4.91 Flash And Four-Board Test
...
expected version guard: v4.4.91
...
```text
logs/burn/v4.4.91_<timestamp>/run_summary.txt
logs/burn/v4.4.91_<timestamp>/<PORT>.log
logs/burn/v4.4.91_<timestamp>/<PORT>.command.txt
```
...
$logDir = "E:\codex_documents\sle\logs\live\v4.4.91_four_board_com16_leader_$ts"
...
- COM16 reports `fw=v4.4.91` and `runtimeRole=leader`.
- COM13/COM17/COM18 report `fw=v4.4.91` and `runtimeRole=member`.
...
Remote Ubuntu build passed and produced a `v4.4.91` package. Flash and four-board hardware validation have not been run yet for v4.4.91.
